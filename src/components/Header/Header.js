...
import {DribbbleIcon,GithubIcon, LinkedinIcon,MoonIcon,SunIcon, TwitterIcon} from "../Icons.js" 
import siteMetadata from '@/src/utilitities/siteMetaData'
import {useThemeSwitch} from "../Hooks/useThemeSwitch"
import { cx } from '@/src/utilitities'
...
const Header = () => {
  ...
  <header className='w-full p-4 px-5 sm:px-10 flex items-center justify-between'>
    ...
    <button className='inline-block sm:hidden z-50' onClick={toggle} aria-label="Menu">
      <div className='w-6 cursor-pointer transition-all ease duration-300'>
        <div className='relative'>
          <span  style={{transForm: click ? "rotate(-45deg)translateY(0)": "rotate(0deg) translaeY(6px)"}} className='absolute w-full h-0.5 rounded transition-all ease duration-200 bg-dark dark:bg-light top-0 inline-block'>
            &nbsp;
          </span>

          <span  style={{opacity: click ? 0 : 1}} className='absolute w-full h-0.5 rounded transition-all ease duration-200 bg-dark dark:bg-light top-0 inline-block'>
            &nbsp;
          </span>
    ...
    style={{
      ...
      "bg-light text-dark" )}
      aria-label="theme-switcher"A
      >
        {mode === "light" ? <SunIcon className={"fill-dark"}/> : <MoonIcon className={"fill-dark"}/>}
      </button>
    </nav>

    <nav className=" w-max py-3 px-8 border border-solid border-orange-500 rounded-full font-medium capitalize  items-center hidden sm:flex
        fixed top-6 right-1/2 translate-x-1/2 bg-light/80 backdrop-blur-sm z-50">
            <Link href="/" className="mr-2">Home</Link>
            <Link href="/about" className="mx-2">About</Link>
            <Link href="/contact" className="mx-2">Contact</Link>
